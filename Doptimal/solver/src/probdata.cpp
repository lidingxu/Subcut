...
SCIP_Real ProbData::evalRay(
	...
	return obj +  emptyvalue - underobj;
}
...
SCIP_Real ProbData::evalUnderSub(
   ...
   const vector<SCIP_Real> & varval  /**< vector value */
	...
	for(int i = 0; i < w_valinds.size(); i++){
		...
		w_indicator[id] = 1;
		SCIP_Real current_val = orcaleValue(w_indicator);
...
SCIP_Real ProbData::evalUnderSubWith(
	...
	for(int i = 0; i < w_valinds.size(); i++){
		...
		prev_val = current_val;
		underobj += coeff * w_valinds[i].first;
		//test_val += w_val[id] * w_coeffs[id];
		//printf("%f %f %f %f \n", test_val, underobj,  w_val[id], w_valinds[i].first);
	}
	return underobj;
}
...
SCIP_Real ProbData::orcaleValue(
	...
	SCIP_Real result = 0;
	mat C = A * diagmat(values) * A.t() + E;
	bool success = log_det_sympd(result, C);
	assert(success);
	return result / dim;
}
...
SCIP_RETCODE ProbData::createInitial(
	...
	bool nonlinear_obj = true; // for test
	...
	// compute initial submodular value
	vec v(numvars, fill::randu);
	std::fill(v.begin(), v.end(), 1);
	fullvalue = orcaleValue(v);
	std::fill(v.begin(), v.end(), 0);
	emptyvalue = orcaleValue(v);

	SCIPdebugMessage("empty value : %f %f\n", emptyvalue, 2 * log(epsilon));
