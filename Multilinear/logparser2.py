...
scales = open("scales.txt", "r").readlines()
scale_map = {}

for scale in scales:
    scale = scale.split(" ")
    scale_map[scale[0]] = float(scale[1])
...
for row_ind in range(len(df)):
    ...
    if row["name"].find("autocorr") != -1:
        entry = {}
        if row["name"] not in scale_map:
            continue
        scale = scale_map[row["name"]]  
        entry["instance"] = "scaled" + row["name"]
        entry["opt"] = float(row["primalbound"]) / scale
...
def printStat(activate, setting, pclass, stat):
    ...
    s = [ str(round(stat[display_key], 3)) + " & " for display_key in display_keys]
...
for activate in activates:
    for setting in settings:
        ...
        #if activate == "a" and setting == "icuts":
    ...
    for j in range(0,3):
        ...
        for k in range(num):
            ...
            if data[(activates[i], pair[0])][k] > data[(activates[i], pair[1])][k]:
                ...
                wins[1] += 1
        print(names[pair[1]], len(data[(activates[i], pair[1])]), names[pair[0]], len(data[(activates[i], pair[0])]) )
