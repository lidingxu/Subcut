   ...
   )
{
   ...
   std::fill(sepadata->vray.begin(), sepadata->vray.end(), 0);
   for(int j = 0; j < raylen; j++){
      int i = rayidx[j];
      sepadata->vray[i] = ray[j];
      if(i < numvars)
         iszero = false;
   }
   ...
   SCIP_Real t;
   if(iszero){
      if(SCIPisFeasZero(scip, sepadata->vray[numvars])){
         t = SCIPinfinity(scip);
      }  
      else{
         t = - sepadata->gamma / sepadata->vray[numvars];
      }
      assert(!SCIPisFeasNegative(scip, t));
      return t;
   }
   ...
   SCIP_Real gamma_t = probdata->evalRay(scip, sepadata->vray, sepadata->vsol, ub);
   if(gamma_t > 0){
      return SCIPinfinity(scip);
   }

   int i = 0;
   for(; i < BINSEARCH_MAXITERS; ++i )
   {
      gamma_t = probdata->evalRayWith(scip,  sepadata->vray,  sepadata->vsol, t, gradient); 
   ...
   )
{
   ...
   int numvars = probdata->numvars;
   if(sepadata->branch_opt){
      probdata->useBranch(scip);
   }

   // check whether to separate
   if(sepadata->vray.size() == 0){
      sepadata->vray = vector<SCIP_Real>(numvars + 1);
      sepadata->vsol = vector<SCIP_Real>(numvars + 1);
   }

   for(int i = 0; i < numvars; i++){
      sepadata->vsol[i] = SCIPvarGetLPSol(probdata->bin_vars[i]);
   }
   sepadata->vsol[numvars] = SCIPvarGetLPSol(probdata->obj_var);  
   SCIP_Real gamma_t = probdata->evalRay(scip, sepadata->vsol, sepadata->vsol, 0);      
   SCIP_Real absval = fmax(fabs(sepadata->vsol[numvars]), fabs(sepadata->vsol[numvars] - gamma_t));
   if(gamma_t <= 0.005 * absval){
      return SCIP_OKAY;
   }
   sepadata->gamma = gamma_t;
   ...
   if( success )
   {
      ...
      if( SCIPgetRowMaxCoef(scip, row) / SCIPgetRowMinCoef(scip, row) / SCIPgetCutEfficacy(scip, NULL, row) < 1e3 )
      {
         ...
         printf(" -> found maxquad-free cut <%s>: act=%f, lhs=%f, norm=%f, eff=%f, min=%f, max=%f (range=%f)\n\n",
               ...
               SCIPgetRowMaxCoef(scip, row)/SCIPgetRowMinCoef(scip, row) / SCIPgetCutEfficacy(scip, NULL, row)); 
